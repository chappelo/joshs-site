...
const $$Index = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "JOSHUA" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="flex flex-col px-4"> <h1 class="text-6xl md:text-9xl xl:text-[138px] font-kabelB animate-leftslide opacity-0">
JOSHUA
</h1> <div class="py-4 xl:py-8"> <p class="text-lg md:text-xl xl:text-2xl text-primary inline-block animate-rightslide opacity-0">
BRITISH SOFTWARE ENGINEER
</p> <p class="text-lg md:text-xl xl:text-2xl text-primary inline-block animate-rightslide opacity-0">
LIVING IN TOKYO
</p> </div> ${headings.map(({ text, link }) => {
    return renderTemplate`<a${addAttribute(link, "href")} class="font-kabelB text-2xl md:text-3xl xl:text-4xl inline-block group relative w-max leading-relaxed md:leading-relaxed xl:leading-relaxed animate-leftslide opacity-0"> ${text} <span class="absolute -bottom-0.5 left-0 w-0 transition-all duration-300 h-1 bg-black group-hover:w-full rounded-md"></span> </a>`;
  })} <!-- TODO: DO I want this? --> <!-- <footer class="flex flex-col w-full items-center mt-auto">
      <a
...
}, "/root/code/jchapps/astro-website/src/pages/index.astro", void 0);

const $$file = "/root/code/jchapps/astro-website/src/pages/index.astro";
const $$url = "";
